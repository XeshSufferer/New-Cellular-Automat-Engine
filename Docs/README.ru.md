...
**"Я изменил настройки и на экран ничего не выводится!"**

1. Если вы настройки поставили настройки с Delay который равен -1 , для генерации понадобится один раз нажать Enter, первая итерация всегда пустая.

2. Если вам не помог совет номер 1 и вы используете Json пользовательские правила, то возможно вы выбрали нереализуемые условия

3. Если вы **УВЕРЕНЫ** что условия реализуемы, то возможно вам стоит удалить папку `CustomRules`, чтобы обнулить все пользовательские правила (**ВНИМАНИЕ:** При удалении папки все пользовательские правила удалятся, а папка восстановится с шаблоном пользовательского правила правила )

4. Если и 3 совет вам не помог, вы можете удалить файл `settings.json` чтобы сбросить настройки. После запуска файл настроек сгенерируется сам автоматически

**System.Text.Json.JsonException: The JSON value could not be converted to Automat.StandardRules.Rule**

- Вы использовали неправильное название встроенного правила в файле `settings.json`. На данный момент существуют встроенные правила "Rule22", "Rule30", "Rule110"

**Condition is invalid ({Какое то из условий в используемом пользовательском правиле})**

- Вы использовали несуществующее условие в пользовательском правиле.

**Rule not found by name: {Имя используемого пользовательского правила правила}**

- Пользовательское правило не найдено по указанному в `settings.json` имени. Маленькое напоминание, название указанного правила должно совпадать с полем `Name` в пользовательском правиле

**Invalid example**

- Если вы столкнулись с этим, то вы разработчик. Если вы разработчик, ознакомьтесь со списком Examples в классе ExampleFactory.

## Проблемы производительности

**Симуляция в консоли нечитаемая!**

- Уменьшите масштаб текста консоли сочетанием клавиш Ctrl и -
- Используйте полноэкранный режим консоли
